...
public class PMCommand implements CommandExecutor {
    ...
    private boolean setHeight(Player p, String[] args) {
        if (!PlotMe.cPerms(p, "PlotMe.admin.setheight")) {
            return true;
        }
        if (!PlotManager.isPlotWorld(p)) {
        ...
        if (args.length < 2 || args[1].equals("")) {
            Send(p, C("WordUsage") + ": " + RED + "/plotme " + C("CommandSetHeight") + " <" + C("WordHeight") + ">");
            return true;
        }
        int height;
        try {
            ...
            Send(p, args[1] + C("MsgNotANumber"));
            return true;
        }
        if (height > 256 || height < 0) {
            Send(p, args[1] + C("ErrOutsideRange"));
        }
        Plot plot = PlotManager.getPlotById(p, id);
        plot.height = height;
    ...
    private boolean addHeight(Player p, String[] args) {
        if (!PlotMe.cPerms(p, "PlotMe.admin.setheight")) {
            return true;
        }
        if (!PlotManager.isPlotWorld(p)) {
    ...
    private boolean subHeight(Player p, String[] args) {
        args[1] = "-" + args[1];
        return addHeight(p, args);
    }

    private boolean setBase(Player p, String[] args) {
        if (!PlotMe.cPerms(p, "PlotMe.admin.setbase")) {
            return true;
        }
        if (!PlotManager.isPlotWorld(p)) {
        ...
        if (args.length < 2 || args[1].equals("")) {
            Send(p, C("WordUsage") + ": " + RED + "/plotme " + C("CommandSetBase") + " <" + C("WordHeight") + ">");
            return true;
        }
        int base;
        try {
